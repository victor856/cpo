...
class PDO_DDPG(RLAlgorithm):
    ...
    def __init__(
        ...
        :param policy: Policy
        :param qf: Reward Q function
        :param qf_cost: Cost Q function
        :param dual_var: dual variable lumbda
        ...
        self.qf_cost_update_method = \
            parse_update_method(
                qf_cost_update_method,
                learning_rate=qf_cost_learning_rate,
            )
    ...
    def init_opt(self):  
        ...
        qval_cost = self.qf_cost.get_qval_sym(obs, action)

        qf_loss = TT.mean(TT.square(yvar - qval))
        qf_reg_loss = qf_loss + qf_weight_decay_term

        qf_cost_loss = TT.mean(TT.square(zvar - qval_cost))
        qf_cost_reg_loss = qf_cost_loss + qf_cost_weight_decay_term
        ...
        qf_cost_updates = self.qf_cost_update_method(
            qf_cost_reg_loss, self.qf_cost.get_params(trainable=True))
        ...
        f_train_qf_cost = ext.compile_function(
            inputs=[zvar, obs, action],
            outputs=[qf_cost_loss, qval_cost],
            updates=qf_cost_updates
        )
    ...
    def do_training(self, itr, batch):
        ...
        obs, actions, rewards, costs, next_obs, terminals = ext.extract(
            batch,
            "observations", "actions", "rewards", "costs", "next_observations",
            "terminals"
        )
